...
//		sdf.applyPattern("yyyy-MM-dd'T'HH:mm:ss'Z'");
	...
	public Connection initDBConnection() throws IOException, SQLException, ClassNotFoundException
	{
		...
		pstAddEntrySubject = cnWorker.prepareStatement("INSERT INTO entries_subjects (subject_id, language_id, entry_id, autogenerated) VALUES (?, ?, ?, false)");
...
//            		Logger.info("Adding tags for entry: " + entry.getTag());
            		addSubjectsForEntry(nEntryID, nDefaultLanguageID, entry.getTagsAsArray(" "));
            	}
            }
...
//		Logger.info("Error occured retrieving " + feedInfo.sURI + "\n" + e);
	...
	private void addSubjectsForEntry(int nEntryID, int nLanguageID, String[] asSubjects)
	{
		try 
		{
			hsEntrySubjects = new HashSet<String>();
			for (int nSubject = 0; nSubject < asSubjects.length; nSubject++){
				addEntrySubject(nEntryID, nLanguageID, asSubjects[nSubject]);
			}
		} catch(SQLException e) {
	...
	private void addEntrySubject(int nEntryID, int nLanguageID, String sSubject) throws SQLException
	{
...
//						pstAddEntrySubject.executeBatch();
	...
	private int addOrUpdateRSSEntry(PreparedStatement pst, FeedInfo feedInfo, SyndEntry entry, DCModule dcm, DCTermsModule dctm, String sURI, String[] asSubjects, Timestamp updatedAt, String sOAIIdentifier, int nEntryID, int nLanguageID) throws SQLException
	{
    	...
    	{
    		addSubjectsForEntry(nEntryID, nLanguageID, asSubjects);
		}
